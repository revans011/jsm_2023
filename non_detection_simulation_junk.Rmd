...
```{r}
b0<- 1   #intercept exp(bb0) is the baseline rupture rate more or less
b1<- 1  #slope
num_wrong<-15
...
predisposition.orig<-c(runif(250,0,0.95),runif(250,0.05,1))

predisposition<-predisposition.orig
...
observed.rupture<-naive.rupture
 ...
 observed.rupture[
   sample(which(observed.rupture==1 & predisposition>0.4),num_wrong)
   ]<-0
...
m2.coef<-c(m2.coef,coef(summary(m2))[,1][2])
