...
out3<-1-(out3==1) #==1 is disagree, 1- == agree
...
#  parmlist<-list(
# c(rep(3000,32)),             #interations; num.iter
...
#  )
  ...
  parmlist<-list(
c(rep(500,16)),             #interations; num.iter
...
#  parmlist<-list(
# c(rep(3000,4)),             #interations; num.iter
# c(120,120,120,120),  #cases; n.cases
# c(60,60,60,60),  #controls unlabeled
# c(0.15,.1,.01,.05),  #non-detection proportion
# c(0.38,0.38,0.38,0.38) #treat effect
#  )
 
#  parmlist<-list(
# c(rep(6000,2)),             #interations; num.iter
# c(120,120),  #cases; n.cases
# c(60,60),  #controls unlabeled
# c(.1,.01),  #non-detection proportion       
# c(0.38,0.38) #treat effect
#  )
...
zoo1 <- goo %>% group_by(n.cases
                        ...
                        ,trt.eff
                        ,prop.nondetect) %>% 
  dplyr::summarise(n.nodetec=median(n.nondetect)
...
zoo2 <- goo %>% group_by(n.cases
                        ...
                        ,trt.eff
                        ,prop.nondetect) %>% 
  dplyr::summarise(case.tru=mean(n.case.tru)
                   ,cntl.tru=mean(n.cntl.tru)
                   ,cas_ctrl_difz=mean(n.cases-n.controls.unlabeled)
                   ,cas_ctrl_diftru=mean(n.case.tru-n.cntl.tru)
                   ,casz_casetru=mean(n.cases-n.case.tru)
                   ,n.nodetec=mean(n.nondetect)
                   ,p.agree=mean(p.agree)
                   ,p.pu=mean(pu)
                   ,p.tru=mean(tru)
                   ,pdiff=mean(abs(pu-tru))
                   ) 

zoo1 %>% ungroup() %>% arrange(pdiff) %>%
  kable(caption = "pdiff ") %>% kable_styling(full_width = FALSE) %>% kable_classic()
