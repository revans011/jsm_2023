...
df <- data.frame(
  weight=c(rnorm(10000, mean=0, sd=1),
...
p<-p+ geom_vline(aes(xintercept=mean(weight)),
            color="blue", linetype="dashed", linewidth=1)+theme(
...
p <- ggplot(df, aes(x=weight,line=z,color=z)) + 
  geom_density(bw=1,  show.legend = FALSE,size = 1)+theme(
  axis.text.x = element_blank(),
  axis.text.y = element_blank(),
  axis.ticks = element_blank())
...
p<-p+ 
  ...
  geom_vline(aes(xintercept=mean(weight[1:14000])),
            color="purple", linetype="dashed", linewidth=1)+theme(
  axis.text.x = element_blank(),
  axis.text.y = element_blank(),
  axis.ticks = element_blank(),
   axis.title = element_blank())+ scale_color_manual( values = c("red","blue"))
