...
# this is a poor choice for variable name
x.tru.control <- c(
    rnorm(unlab.pos, mean = eff.sz),
    rnorm(n.tru.unaff)
)
...
naive.dat <- data.frame(
    X = as.numeric(c(x.tru.affected, x.tru.control) > runif(200)),
    y = naive.labels
)
...
parmlist <- list(
    rep(10000, 15), # interations; num.iter
    rep(100, 15), # n.cases
    rep(100, 15), # number unlabeled
    rep(.5, 15), # eff.sz for normals
    seq(from = 0, to = 0.1, length.out = 15)
