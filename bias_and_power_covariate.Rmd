...
temp.out4 <-  pwr::pwr.t2n.test(n1 = n.cases,
                  n2 = n.naiv.control,
                sig.level = 0.05,
...
smry.otpt <- big.otpt %>% group_by(n.cases,n.naiv.control,prp.nondet,eff.sz) %>% 
  dplyr::summarise(prp.nondet=mean(prp.nondet)
                   ,eff.sz=mean(eff.sz)
                   ,naive.or.mn=mean(naive.or)
                   ,naive.p.mn=mean(naive.p)
...
smry.otpt %>% ungroup() %>% 
  select(-c(naive.or.mn,naive.p.mn,naive.p.sd, eff.sz)) %>%   arrange(prp.nondet,naive.power)

smry.otpt %>% ungroup() %>% select(-c(naive.or.mn,naive.p.mn,naive.p.sd, eff.sz)) %>%   arrange(naive.power)
...
smry.otpt %>% ungroup() %>% select(n.cases,n.naiv.control,prp.nondet,naive.power) %>%  reframe(prp.nondet=prp.nondet,zzz=naive.power/mean(naive.power[prp.nondet==0]))

```


```{r}
df.plot<-smry.otpt %>% ungroup() %>% select(n.cases,n.naiv.control,prp.nondet,naive.power) %>%  reframe(prp.nondet=prp.nondet,zzz=naive.power/mean(naive.power[prp.nondet==0])) %>% 
...
ggplot(data=df.plot, aes(x=prp.nondet, zzz)) +
  geom_line(color="red")+
  geom_point()+xlab("Percent of non-detected positives in the control group")+
  ylab("Relative power")+
