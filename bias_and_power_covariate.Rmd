...
tru.labels<- as.numeric(preds>runif(n.dogs))
...
naive.labels[tru.labels==1]<- as.numeric(runif(sum(tru.labels))>prp.nondet) # affected get reassigned to zero if runif<nondet prop, one if
...
out1<- c(out1,rel.bias)


 
} #loop

return(list(n.cases=n.dogs,
...
zoo1 <- goo %>% group_by(prp.nondet) %>% 
  dplyr::summarise(prp.nondet=mean(prp.nondet)
                   ,mn.rel.bias=mean(bias.percent)
                   )

print(difftime(Sys.time(),now,units="mins"))
return(pandoc.table(zoo1 %>% ungroup() %>% arrange(desc(mn.rel.bias))
...
print("sample sizes from the papers: healey (216)")
 parmlist<-list(
c(rep(55,4)),             #interations; num.iter
c(161,150,108,121),   #n.dogs
